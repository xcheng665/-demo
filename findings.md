...
## 横向浮动 OGL 画廊（2026-08-30）

- 用户已确认将 `/other` 页的实践卡片替换为横向 OGL 画廊：`bend={0}`、`scrollEase={0.02}`，不要弧形排列。
- 需要保留现有五项实践数据和下方详情切换；画廊滑动与详情按钮应双向同步。
- `ogl` 已存在于项目依赖。提供的 `CircularGallery` 是实现基础，但需要改造成项目内的 TypeScript 组件，并使用本地字体和实际卡片数据。
- 全局鼠标跟随光晕的删除是独立的未提交改动，不能混入画廊提交。
