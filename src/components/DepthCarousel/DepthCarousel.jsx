...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    index => {
      ...
      if (index === focusRef.current) {
        onItemActivateRef.current?.(index, data[index]);
        return;
      }
      setFocus(index, true);
    },
  ...
  useEffect(() => {
    ...
    if (!autoplay || isAutoplayPaused || reducedRef.current || count < 2) return;
    const root = rootRef.current;
    let hovered = false;
    let focused = false;
    const stop = () => {
  ...
  }, [autoplay, autoplayDelay, count, isAutoplayPaused, navigateBy]);
  ...
  return (
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          <button
            ...
            aria-label={active === i ? `Open ${item.alt || `slide ${i + 1}`}` : `Select ${item.alt || `slide ${i + 1}`}`}
          ...
          >
            <img
              ...
              fetchPriority={active === i ? 'high' : 'auto'}
              draggable={false}
            />
            <span
      ...
      {autoplay && showAutoplayToggle && count > 1 && (
        <button
          type="button"
          className="depth-carousel__toggle"
          aria-label={isAutoplayPaused ? 'Resume carousel autoplay' : 'Pause carousel autoplay'}
